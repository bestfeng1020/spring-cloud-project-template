define(function () {
    ...
    return {
        ...
        listToTree: function (list, rootPredicate) {
            var cache = {};
            rootPredicate = rootPredicate || function (node) {
            ...
            $(list).each(function () {
                ...
                cache[this.parentId].push(this);
            });
            var root = [];

            $(list).each(function () {
                this.children = cache[this.id];
                if (rootPredicate(this)) {
                    root.push(this);
                }
            });
            return root;
                ...
                require(["message", "logger"], function (message, logger) {
                    try {
                        ...
                        if (res.status === 401) {
                            ...
                            message.showTips(res.message, "danger");
        ...
        createActionLink: function (title, text, onclick) {
            if (!window.action_countter) window.action_countter = 0;
            var callId = "action_" + (++window.action_countter);
            window[callId] = onclick;
            return ["<a href='javascript:void(0)'", " onclick='", callId, "()'", " title='", title, "'", " class=''", ">" + text + "</a>"].join("");
        },
