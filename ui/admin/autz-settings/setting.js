...
var allSupportDataAccessTypes = [
    {id: "DENY_FIELDS", text: "禁止操作字段"},
    {id: "OWN_USER", text: "仅限本人"},
    {id: "POSITION_SCOPE", text: "仅限本人及下属"},
    {id: "DEPARTMENT_SCOPE", text: "所在部门"},
    {id: "ORG_SCOPE", text: "所在机构"}
    // {id: "CUSTOM_SCOPE_ORG", text: "自定义设置-机构"},
    // {id: "CUSTOM_SCOPE_DEPARTMENT", text: "自定义设置-部门"},
    // {id: "CUSTOM_SCOPE_POSITION", text: "自定义设置-岗位"}
];

var dataAccessNameMapping = {
    OWN_USER:"仅限本人",
    POSITION_SCOPE:"仅限本人及下属",
    DEPARTMENT_SCOPE:"所在部门",
    ORG_SCOPE:"所在机构"
}
    ...
    $(".data-access-table input:checked").each(function () {
        ...
        if (nowSelectPermission) {
            ...
            function applyDataAccess(per) {
                ...
                if (per.type === "action") {
                    var accessConfig = {
                        config: {scopeType: scopeType, scopeTypeName:dataAccessNameMapping[scopeType], children: true},
                        type: "SCOPE_BY_USER",
                        action: per.action
                    };
                    ...
                    //     var list = [];
                    //     for (var type in tmp) {
                    //         list.push({type: "SCOPE_BY_USER"});
                    //     }
                    //     accessConfig.config = {scopeType:scopeType,scope: list};
                    // }
                    if (scopeType !== 'ALL') {
...
function initDataAccessSetting(e) {
    ...
    if (node.dataAccesses) {
        $(node.dataAccesses).each(function () {
            var type = typeof this.config === 'object' ? this.config.scopeType : JSON.parse((this.config || "{}")).scopeType;
            if (type) {
                $("input[access-type=" + type + "]").prop("checked", true);
                if (type.indexOf("CUSTOM_SCOPE") !== -1) {
                    customGrid.setEnabled(true);
                    customGrid.setShowCheckBox(true);
                    //加载选中的自定义信息
...
function initData(type, settingFor) {
    ...
    require(["request", "miniui-tools", "message","authorize"], function (request, tools, message,authorize) {
        ...
        var url = "menu/user-own/list?paging=false";
        if(authorize.user.username==='admin'){
            url="menu/no-paging"
        }
