    ...
    require(["request", "message"], function (request, message) {
        ...
        request.get("permission/" + id, function (response) {
            ...
            if (response.status == 200) {
                ...
                mini.get('parent-permission-grid').setData(response.result.parents);
...
function getDataAndValidate() {
    ...
    if (data.supportDataAccessTypes.length > 0) {
        data.supportDataAccessTypes.push("SCOPE_BY_USER");
    }
    var parentPermission = mini.get('parent-permission-grid').getData();
    $(parentPermission).each(function () {
        if (this.actions)
            this.actions = this.actions.split(",");
    });
    data.parents = parentPermission;
