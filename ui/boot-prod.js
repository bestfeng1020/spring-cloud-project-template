...
//api服务地址
window.API_BASE_PATH = window.location.protocol + (window.location.hostname.indexOf('gcable.cn')!==-1 ? "//networking.gcable.cn/api/" : "//api.iot.zmcsoft.com/api/");
...
window.router = [
    ...
    {
        test: "^(user-server|device-server|plugins-server|query-server|networking-server|home-server).*$",
        prefix: ""
    },
    {
        test: "^(plugin).*$",
        prefix: "plugins-server/"
    },
    {
        test: "^(device).*$",
        prefix: "device-server/"
    },
    {
        test: "^(file).*$",
        prefix: "file-server/"
    ...
    {
        test: "^(user|role|organizational|department|position|person|district|authorize|menu|permission|autz-setting|oauth2|dashboard).*$",
        prefix: "user-server/"
    },
    {
        test: "^(database|dev|datasource|redis).*$",
        prefix: "devops-server/"
    },
    {
        test: "^(networking).*$",
        prefix: "networking-server/"
    },
    {
        test: "^(home).*$",
        prefix: "home-server/"
...
function importMiniui(callback) {
    function doImport() {
        ...
        var mode = getParameter("_mode", getCookie("mode", window.miniui_mode || 'my'));

        // window.outerHeight > 1000 ? "large" : "medium");

        function loadMini() {
            if (window.ActiveXObject) {
                require(["plugin/tools/jquery.xdomainrequest.min"]);
            }
            if (!window.doLogin) {
